...
class LocalCluster(object):
    ...
    def install(cls, warehouse, name, version, **databases):
        ...
        initial_discovery_members = ",".join("localhost:%d" % (10050 + 100 * i)
        ...
        for i, database in enumerate(core_databases):
            ...
            install.set_config("dbms.connector.bolt.listen_address", ":%d" % (10087 + 100 * i))
            install.set_config("dbms.connector.http.listen_address", ":%d" % (10074 + 100 * i))
            install.set_config("dbms.connector.https.listen_address", ":%d" % (10073 + 100 * i))
            install.set_config("causal_clustering.database", database)
            install.set_config("causal_clustering.discovery_listen_address", ":%d" % (10050 + 100 * i))
            ...
            install.set_config("causal_clustering.raft_listen_address", ":%d" % (10070 + 100 * i))
            install.set_config("causal_clustering.transaction_listen_address", ":%d" % (10060 + 100 * i))
        for i, database in enumerate(read_replica_databases):
            ...
            install.set_config("dbms.backup.enabled", False)
            install.set_config("dbms.connector.bolt.listen_address", ":%d" % (20087 + 100 * i))
            install.set_config("dbms.connector.http.listen_address", ":%d" % (20074 + 100 * i))
            install.set_config("dbms.connector.https.listen_address", ":%d" % (20073 + 100 * i))
            install.set_config("causal_clustering.database", database)
            install.set_config("causal_clustering.discovery_listen_address", ":%d" % (20050 + 100 * i))
            install.set_config("causal_clustering.expected_core_cluster_size", 3)
            install.set_config("causal_clustering.initial_discovery_members", initial_discovery_members)
            install.set_config("causal_clustering.raft_listen_address", ":%d" % (20070 + 100 * i))
            install.set_config("causal_clustering.transaction_listen_address", ":%d" % (20060 + 100 * i))
