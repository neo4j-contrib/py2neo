...
def test_unique_path_not_unique_raises_cypher_transaction_error_in_transaction(graph):
    ...
    tx.enqueue("CREATE (a), (b) RETURN a, b")
    results = tx.process()
    result = results[0]
    record = result[0]
    parameters = {"A": record.a._id, "B": record.b._id}
    tx.enqueue("START a=node({A}), b=node({B}) CREATE (a)-[:KNOWS]->(b)", parameters)
    tx.enqueue("START a=node({A}), b=node({B}) CREATE (a)-[:KNOWS]->(b)", parameters)
    tx.enqueue("START a=node({A}), b=node({B}) CREATE UNIQUE (a)-[:KNOWS]->(b)", parameters)
    try:
        tx.commit()
