...
class SingleCogTestCase(MovieGraphTestCase):
    ...
    def test_equality(self):
        ...
        second_cog = self.new_keanu_cog()
        assert first_cog == second_cog

    def test_inequality(self):
        first_cog = self.new_keanu_cog()
        second_cog = Person.select(self.graph, "Carrie-Anne Moss").first().__cog__
        assert first_cog != second_cog

    def test_inequality_with_other_types(self):
        first_cog = self.new_keanu_cog()
        assert first_cog != "this isn't even a cog"
    ...
    def test_can_clear_objects_of_type(self):
        # given
        keanu_cog = self.new_keanu_cog()
        keanu_cog.__db_pull__(self.graph)
    ...
    def test_can_get_relationship_property_using_default(self):
        # given
        keanu_cog = self.new_keanu_cog()
        keanu_cog.__db_pull__(self.graph)
        matrix_reloaded = Film('The Matrix Reloaded')
        ...
        # then
        foo = keanu_cog.get("ACTED_IN", matrix_reloaded, "foo", "bar")
        assert foo == "bar"
