...
class IndirectURITest(unittest.TestCase):
    ...
    def test_graph_db_uri_construction(self):
        ...
        self.assertEqual("http://localhost:7474/db/data", graph_db._base_uri)
        self.assertEqual("/", graph_db._relative_uri)

    def test_node_uri_construction(self):
        node = neo4j.Node("http://localhost:7474/db/data/node/123")
        self.assertEqual("http://localhost:7474/db/data/node/123", node._uri)
        self.assertEqual("http://localhost:7474/db/data", node._base_uri)
        self.assertEqual("/node/123", node._relative_uri)
    ...
    def test_relationship_uri_construction(self):
        ...
        self.assertEqual("http://localhost:7474/db/data", rel._base_uri)
        self.assertEqual("/relationship/123", rel._relative_uri)
