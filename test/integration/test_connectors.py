...
def test_bolt_connection_pool_usage_for_autocommit(connector):
    ...
    cursor = connector.run_prog(None, "RETURN 1")
...
def test_keys(connector):
    cursor = Cursor(connector.run_prog(None, "RETURN 'Alice' AS name, 33 AS age"))
    expected = ["name", "age"]
    actual = cursor.keys()
...
def test_records(connector):
    cursor = Cursor(connector.run_prog(None, "UNWIND range(1, $x) AS n RETURN n, n * n AS n_sq", {"x": 3}))
...
def test_stats(connector):
    cursor = Cursor(connector.run_prog(None, "CREATE ()", {}))
