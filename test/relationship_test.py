...
class RelateTestCase(unittest.TestCase):
    ...
    def test_complex_relate(self):
        ...
        batch.get_or_create_relationship((alice, "IS~MARRIED~TO", bob, {"since": 1996}))
        #batch.get_or_create((alice, "DISLIKES", carol, {"reasons": ["youth", "beauty"]}))
        batch.get_or_create_relationship((alice, "DISLIKES!", carol, {"reason": "youth"}))
        rels1 = batch.submit()
        self.assertIsNotNone(rels1)
        self.assertEqual(2, len(rels1))
        batch = neo4j.WriteBatch(self.graph_db)
        batch.get_or_create_relationship((bob, "WORKS WITH", carol, {"since": 2004, "company": "Megacorp"}))
        #batch.get_or_create((alice, "DISLIKES", carol, {"reasons": ["youth", "beauty"]}))
        batch.get_or_create_relationship((alice, "DISLIKES!", carol, {"reason": "youth"}))
        batch.get_or_create_relationship((bob, "WORKS WITH", dave, {"since": 2009, "company": "Megacorp"}))
        rels2 = batch.submit()
